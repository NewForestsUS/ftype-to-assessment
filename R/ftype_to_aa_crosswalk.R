...
reclass_matrix <- aa_lookup_dt[,c("ftype_code", "aa_code")]
...
aa_rast_list <- parLapply(cl, unique_ss, function(x, aa_lookup_dt, supersect_sf, ftype_raster, reclass_matrix){
  library(sf)
  library(magrittr)
  library(raster)
  library(data.table)
  ...
  reclassify(ftype_masked, reclass_matrix)
}, aa_lookup_dt, supersect_sf, ftype_raster, reclass_matrix)
stopCluster(cl)
...
# writeRaster(aa_mosaic, paste0(raster_dir, "CONUS-assessment-areas-240m.tif"))
...
# Write each to disk
# writeRaster(cp_lo_rast, paste0(raster_dir, "CONUS-common-practice-low-240m.tif"))
# writeRaster(cp_hi_rast, paste0(raster_dir, "CONUS-common-practice-hi-240m.tif"))
